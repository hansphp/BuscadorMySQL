...
if(isset($_GET['type'])){
	if($_GET['type'] == 'table'){
		...
		$result = $SQL->consulta("SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, DATA_TYPE, IFNULL(CHARACTER_MAXIMUM_LENGTH, IFNULL(NUMERIC_PRECISION, DATETIME_PRECISION)) MAX_LEN, COLLATION_NAME, COLUMN_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = '{$_GET['database']}' AND TABLE_SCHEMA <> 'information_schema' AND TABLE_NAME = '{$_GET['table']}' ORDER BY ORDINAL_POSITION");
	}
}
